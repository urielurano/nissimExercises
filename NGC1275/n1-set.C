...
Float_t cc = 10;
 ...
 fun4 = new TF1("fun4"," [0]*(((1e-1/[2])^(-0.5)*(x/[2])^(4/3)*(5e-4/[2])**((3-[1])/2)*(x>1e-6)*(x<5e-4))+((1e-1/[3])^(-0.5)*(x/[3])**((3-[1])/2)*(x>=5e-4)*(x<1e-1)) + ((x/[4])**((2-[1])/2))*(x>=1e-1)*(x<10))", 1e-6,100); //pgamma interaction
 // Se fija el valor inicial de las energías de normalización:
  fun4->SetParameter(2,0.00004);
  fun4->SetParameter(3,10);
  fun4->SetParameter(4,10);
