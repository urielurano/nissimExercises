...
def Par1(a,b,c):
    ...
    f2 = interp1d(x4, y4, kind='quadratic')#quadratic
    xnew = np.linspace(6e-7, 4e-4, 100000) # you can change the 100 here to a smaller number if you want less 'smoothness'

    #interp = pchip(x4, y4)
    #xx = np.linspace(6e-7, 4e-4, 100001)
...
def xx(aa,xmin, xmax):
    ...
    for ii in arange(-6.3, -2, 1.2):
        ...
        if(E_m < Egamma and  Egamma < E_c ):
            C2=fl*pow(Egamma/E_m,-(2.52-3)/2)
        if(Egamma >E_c):
            C3=fl*pow(E_c/E_m,-(2.52-3)/2)*pow(Egamma/E_c,-(2.52-2.0)/2)
        C=C1+C2+C3
...
def make_plot1():
    ...
    if ifile:
        ...
        mg.SetTitle('')
        fun1 = TF1('fun1','[0]*(((x/1.8e-6)^(4/3)*(x>1e-6)*(x<1.8e-6))+((x/1.8e-6)**((3-2.52)/2)*(x>=1.8e-6)*(x<3.48e-5)) + ((3.48e-5/1.8e-6)**((3-2.52)/2)*(x/3.48e-5)**((2-2.52)/2))*(x>=3.48e-5)*(x<1e-4))', min(x1), max(x1))
        gr1.Fit('fun1',"Q")
        #mg.SetLineColor(6) 
        fun1.Draw('L same')
