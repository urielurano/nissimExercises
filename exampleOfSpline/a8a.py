...
def Par1(a,b,c):
    ...
    s = interpolate.InterpolatedUnivariateSpline(x4,y4,k=2)
    #f2 = interp1d(x4, y4, kind='nearest')
    xnew=np.linspace(6e-7, 6e-4, 1001)
    ynew=s(xnew)
...
def make_plot1():
    ...
    if ifile:
        ...
        mg.SetTitle('')
        fun1 = TF1('fun1','[0]*(((x/1.65e-14)**((3-2.52)/2)*(x>=1.5e-6)*(x<1e-4)) + ((1e-4/1.65e-14)**((3-2.52)/2)*(x/1e-4)**((2-2.52)/2))*(x>=1e-4)*(x<1.2e-4))', min(x1), max(x1))
        gr1.Fit('fun1',"Q")
        #mg.SetLineColor(6) 
        fun1.Draw('L same')
