...
def Par1(a,b,c):
    ...
    #coef=polyfit(x,y,20)
    #polynomial = poly1d(coef)
    #rbf=Rbf(x4, y4)
    #ius = interpolate.InterpolatedUnivariateSpline(x4, y4,k=2)
    #xi=np.linspace(6e-7, 6e-4, 1000000)
    #fi=rbf(xi)
    #yi = polynomial(xi)
    
    #s = interpolate.InterpolatedUnivariateSpline(x4,y4)
    #f2 = interp1d(x4, y4, kind='nearest')
    #xnew=np.linspace(6e-7, 6e-4, 100001)
    #ynew=s(xnew)
